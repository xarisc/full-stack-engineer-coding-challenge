...
export class AddPricingCatalogs1779321600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // concurrency guard: partial index to ensure only one DRAFT version per craftsman and trade
    await queryRunner.query(`
        CREATE UNIQUE INDEX ON pricing_service.catalog_versions (craftsman_id, trade)
        WHERE status = 'DRAFT'
    `);
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'vat_rate', type: 'numeric', precision: 10, scale: 4, isNullable: true },
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'position_keys', type: 'varchar[]', isNullable: true },
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'DROP INDEX IF EXISTS pricing_service.uniq_one_published_per_craftsman_trade',
    );
